...
class DataGenerator:
    def __init__(self, filepath, mode, batch_size, num_classes):
        ...
        if mode == 'train':
            self.data_size = 42948
        else:
            self.data_size = 10892
    ...
    def write_to_tfrecord(self, filepath, mode):
      with tf.name_scope("write_image"):
        ...
        recordfilenum = 0
        #tfrecords格式文件名
        tf_filepath = 'H:\\shuqian\\amel\\first\\5_tfrecord\\'
        if mode == 'train':
    ...
    def read_from_tfrecord(self, batch_size, num_classes, mode):
      with tf.name_scope("read_image"):
        if mode == 'train':
            files = tf.train.match_filenames_once('H:\\shuqian\\amel\\first\\5_tfrecord\\train_image.tfrecords*')
        else:
            files = tf.train.match_filenames_once('H:\\shuqian\\amel\\first\\5_tfrecord\\test_image.tfrecords*')
