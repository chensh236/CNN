...
train_file = "H:\\shuqian\\resize\\image\\train\\"
val_file = "H:\\shuqian\\resize\\image\\test\\"
...
filewriter_path = "H:\\shuqian\\resize\\code\\graph"
checkpoint_path = "H:\\shuqian\\resize\\code\\checkpoints"
...
def normalize_weight(nparray, alpha=-1):
...
train_writer = tf.summary.FileWriter("H:\\shuqian\\resize\\code\\graph\\train")
test_writer = tf.summary.FileWriter("H:\\shuqian\\resize\\code\\graph\\test")
...
with tf.device('/gpu:0'):
    ...
    with tf.Session(config = tf.ConfigProto(gpu_options = gpu_options)) as sess:
        ...
        for epoch in range(num_epochs):
            ...
            filters = vars['sub_conv/filters:0']
            y_filter= tf.transpose(filters, [3, 2, 1, 0]).eval()
            for i in range(8):
                for j in range(3):
                    y_filter[i][j] = (normalize_weight(y_filter[i][j]))
            update = tf.assign(vars['sub_conv/filters:0'], y_filter.transpose(2, 3, 1, 0))
            sess.run(update)
            ...
            #print (corr, tf.argmax(labels, 1).eval())
